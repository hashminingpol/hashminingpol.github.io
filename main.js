...
function refreshData(){
    ...
    getMyMiners(function(miners){
        ...
        calculateEggBuySimple(web3.utils.toWei('0.1'),function(eggs){
            devFee(eggs,function(fee){
                sellsforexampledoc.textContent='0.1 POL Mining Rigs BUYS ' + formatEggs(eggs-fee) + ' miners';
            });
        });
    });
    updateBuyPrice()
    ...
    prldoc.textContent=window.location.origin+"/index.html?ref="+currentAddr
...
function updateBuyPrice(){
    ...
    calculateEggBuySimple(web3.utils.toWei(trxspenddoc.value),function(eggs){
        devFee(eggs,function(fee){
            eggstobuydoc.textContent=formatEggs((eggs-fee)/3)
        });
    });
}
function mine2(){
